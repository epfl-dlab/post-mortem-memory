...
plot_time_series_for_small_multiples <- function(name, medium, X, x, num_art,
  ...
  text(360, -1, sprintf('%s\n(%s)', gsub('_', ' ', name), medium), adj=c(1,1), cex=1)
...
shifted_power_fit <- function(medium) {
  df <- make_df_for_fit(medium)
  ...
  # Full model.
  model <- nls(log(y) ~ log( N*t^-p + r ), start=list(p=p0, N=N0, r=0),
               data=df, control=list(maxiter=1e8), algorithm='port')
  return(model)
}
...
plot_avg_fraction_of_mentioning_docs <- function(medium, model=NULL, model_type=NULL) {
  if (SAVE_PLOTS) pdf(sprintf('%s/avg_mention_curve_%s.pdf', PLOTDIR, medium), width=4.4, height=3.4,
                      pointsize=8, family='Helvetica', useDingbats=FALSE)
  par(mar=c(3,3,1,1))
  if (medium == 'NEWS') {
    x <- x_N
  ...
  mtext(expression(paste('Day ', italic(t), ' since death')), side=1, line=2)
  mtext(expression(paste(Log[10], ' of fraction ', italic(S),'(',italic(t),')', ' of mentioning documents')), side=2, line=2)
  y365 <- y['365']
  arrows(x0=325, x1=355, y0=y365, y1=y365, length=0.05)
  text(325, y365, 'Day 365', adj=c(1.1, 0.5))
  ...
  if (!is.null(model)) {
    ...
    if (model_type == 'Biexponential') {
      ...
      legend <- c(expression(paste('Biexponential fit ', italic(S), '(',italic(t),') = ', italic(u), '(',italic(t),') + ', italic(v), '(',italic(t),')')),
                  expression(paste('Communicative memory ', italic(u), '(',italic(t),')')),
                  expression(paste('Cultural memory ', italic(v), '(',italic(t),')')))
    } else if (model_type == 'Bipower') {
      mem_comm <- N * t^-(p+r)
      mem_cult <- N/(p-q+r)*r * (t^-q - t^-(p+r))
      legend <- c(expression(paste('Bipower fit ', italic(S), '(',italic(t),') = ', italic(u), '(',italic(t),') + ', italic(v), '(',italic(t),')')),
                  expression(paste('Communicative memory ', italic(u), '(',italic(t),')')),
                  expression(paste('Cultural memory ', italic(v), '(',italic(t),')')))
    } else if (model_type == 'Shifted power') {
      mem_comm <- N * t^-p
      mem_cult <- rep(r, length(t))
      legend <- c(expression(paste('Shifted power law ', italic(S), '(',italic(t),') = ', italic(u), '(',italic(t),') + ', italic(v), '(',italic(t),')')),
                  expression(paste('Communicative memory ', italic(u), '(',italic(t),') = ', italic(a)*italic(t)^{-italic(b)})),
                  expression(paste('Cultural memory ', italic(v), '(',italic(t),') = ', italic(c))))
    } else {
    ...
    legend('topright', bty='n',
           legend=c(sprintf('Daily average (%s)', medium), legend),
           seg.len=3,
           pch=c(1,NA,NA,NA),
    ...
    lines(t, log10(mem_cult), lty=1, lwd=2, col=COL_RED)
    lines(t, log10(mem), lty=1, lwd=2, col='black')
    legend('topright', bty='n', legend=expression(paste('Log ', italic(x), '-scale')), inset=c(0.1,0.2))
  }

  if (SAVE_PLOTS) dev.off()
}

plot_model_fit <- function(medium, name, model, predict, log='', mar=c(3,3,1,1)) {
  if (medium == 'NEWS') {
    x <- x_N
  } else if (medium == 'TWITTER') {
    x <- x_T
  ...
  par(mar=mar)
  plot(t, y, type='p', bty='n', lwd=2, xlab='', ylab='', las=0, log=log, col=COL[[medium]])
...
plot_all_model_fits <- function(medium) {
  models <- list(
                 ...
                 "Shifted power"=shifted_power_fit(medium)
                 )
  if (SAVE_PLOTS) pdf(sprintf('%s/all_model_fits_%s.pdf', PLOTDIR, medium),
                      width=6, height=5, pointsize=8, family='Helvetica', useDingbats=FALSE)
  par(mfcol=c(5,4))
  for (batch in 0:1) {
    for (log in c('', 'x')) {
      for (i in batch*5+(1:5)) {
        name <- names(models)[i]
        plot_model_fit(medium, sprintf('(%d) %s', i, name), models[[name]], exp(predict(models[[name]])),
                       log=log, mar=c(3,4,1.5,1))
      }
    }
  }
  par(.default_par)
  # par(fig=c(0,1,0,1), new=TRUE, mar=c(0,0,0,0), xpd=NA, mgp=c(0,0,0))
  segments(0.48, 0, 0.48, 1, col='gray', lwd=2)
  if (SAVE_PLOTS) dev.off()
}
...
plot_comm_vs_cult_mem <- function(medium, coefs, model_type) {
  ...
  if (model_type == 'Biexponential') {
    ...
    u <- function(t, N, p, r, q) N * t^-p
    v <- function(t, N, p, r, q) r
  } else {
    stop("Invalid model type")
  }
  ...
  for (i in 1:length(percs)) {
    idx <- min(which(ratio>=percs[i]))
    points(t[idx], ratio[idx], lwd=3, pch=20, col=cols[i], cex=2)
    segments(-5, ratio[idx], t[idx], ratio[idx], col=cols[i], lty=2)
    segments(t[idx], -5, t[idx], ratio[idx], col=cols[i], lty=2)
  }
  for (i in 1:length(percs)) {
    idx <- min(which(ratio>=percs[i]))
    text(t[idx], ratio[idx], adj=c(-0.1, 0.5), col=cols[i],
         labels=sprintf('%.0f%% after %.0f days', 100*percs[i], ceiling(t[idx])))
  }
  if (SAVE_PLOTS) dev.off()
}
...
plot_age_coeffs_for_death_types <- function(mod, medium, outcome, base_age, ranks,
  ...
  if (outcome == 'peak_mean_boost') {
    ...
    main <- 'Long-term boost (NEWS vs. TWITTER)'
    xlab <- 'Average boost difference'
    outfile <- sprintf('%s/age_coefs_NEWS-VS_TWITTER_%s%s.pdf', PLOTDIR, outcome, suffix)
  } else {
...
```{r, echo=!SUPP_INFO}
shifted_power_model_N <- shifted_power_fit('NEWS')
shifted_power_model_T <- shifted_power_fit('TWITTER')
```

`r '__(a) News__'`

```{r, fig.width=6, fig.height=5, echo=!SUPP_INFO}
plot_avg_fraction_of_mentioning_docs('NEWS', shifted_power_model_N, 'Shifted power')
```

`r '__(b) Twitter__'`

```{r, fig.width=6, fig.height=5, echo=!SUPP_INFO}
plot_avg_fraction_of_mentioning_docs('TWITTER', shifted_power_model_T, 'Shifted power')
```
...
```{r, echo=!SUPP_INFO}
coef(shifted_power_model_N)
```

Twitter:

```{r, echo=!SUPP_INFO}
coef(shifted_power_model_T)
```
\fi
...
```{r alternative_models_N, fig.width=8, fig.height=8, echo=!SUPP_INFO}
...
```{r alternative_models_T, fig.width=8, fig.height=8, echo=!SUPP_INFO, warning=!SUPP_INFO}
plot_all_model_fits('TWITTER')
```

`r figcap(TRUE)`
Comparison of ten models $S(t)$ for fitting empirical mention frequencies, for __(a)__ the news and __(b)__ Twitter.
All $y$-axes are logarithmic.
The left and right plots in each pair show the same fits, the only difference being that the left plots have linear $x$-axes, whereas the right plots have logarithmic $x$-axes.
Fits are nonlinear least-squares fits obtained in log space, i.e., logarithms were taken of both the empirical data and the model $S(t)$ before performing the least-squares optimization (see equation 1 in the paper for the case of the shifted power function).
...
Based on the biexponential function, we define the (novel) bipower function by replacing exponentials with powers:

* Bipower: $S(t) = \frac{N}{p+r-q} \left(  (p-q) t^{-(p+r)} + rt^{-q} \right)$.

By fixing $q=0$ and defining $a=\frac{Np}{p+r}$, $b=p+r$, and $c=\frac{Nr}{p+r}$, we obtain the shifted power function as a special case of the bipower function:

* Shifted power: $S(t) = a t^{-b} + c$.
...
$R^2$ and AIC result in the same ordering of the 10 models, and the ordering is identical across the two media (news and Twitter).
In the figure, the models are numbered in increasing order of goodness of fit.
The shifted power model provides the best fit according to both measures ($R^2$ and AIC) and for both media (news and Twitter), with $R^2=0.989$ for the news and $R^2=0.985$ for Twitter.
The bipower model yields essentially the same fit as the shifted power model, which, as mentioned above, is a special case of the bipower model with $q=0$ fixed (optimal bipower fit: $q=0.0068$ for the news, $q=0$ for Twitter).
...
par(mfrow=c(1,2))
plot_comm_vs_cult_mem('NEWS', coef(shifted_power_model_N), 'Shifted power')
plot_comm_vs_cult_mem('TWITTER', coef(shifted_power_model_T), 'Shifted power')
par(.default_par)
```

`r figcap(TRUE)`
Proportion of cultural vs. communicative memory.
On day $t$ after death, the total collective memory according to the shifted power law fit is $S(t)=u(t)+v(t)$, where $u(t)=at^{-b}$ is the communicative memory, and $v(t)=c$ is the cultural memory.
...
```
`r figcap(TRUE)`
Relative length increase of documents that mention deceased public figures (excluding Twitter posts), with respect to the respective public figure's pre-mortem mean document length, as a function of days since death.
...
We see that, both with and without adjustment, the documents that mentioned a person on the day of their death were on average about 40% shorter than documents that mentioned the person before their death, and that the pre-mortem level is reached again (and then surpassed) after about one month.
...
```
`r figcap(TRUE)`
Distributions of mention time series characteristics (rank-transformed and scaled to the interval $[0,1]$) by notability type, visualized as box plots, for __(a)__ the news and __(b)__ Twitter.

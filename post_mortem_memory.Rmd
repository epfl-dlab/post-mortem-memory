...
plot_model_fit <- function(medium, name, model, predict, log='') {
  ...
  t <- 1:max_day
  eps <- min(x[x>0], na.rm=TRUE)
  y <- colMeans(log10(x[,colnames(x) %in% t] + eps), na.rm=TRUE)
  par(mar=c(3,3,1,1))
  plot(t, y, type='p', bty='n', lwd=2,
       xlab='', ylab='',
       col=COL[[medium]], las=0, log=log)
  mtext('Days since death', side=1, line=2, cex=0.6)
  mtext('Mention frequency [log10]', side=2, line=2, cex=0.6)
  lines(t, log10(predict), lwd=2)

  if (log=='') text(200, max(y), name, adj=c(0.5,1), cex=1.7)
  else text(400, max(y), sprintf('AIC = %.0f', AIC(model)), adj=c(1,1), cex=1)
}

plot_all_model_fits <- function(medium) {
  models <- list("Biexponential"=biexp_fit(medium),
                 "Log-normal-inspired"=lognorm_fit(medium),
                 "Log-normal-inspired\n(constrained)"=lognorm_constrained_fit(medium),
...
```{r alternative_models_N, fig.width=5, fig.height=11, echo=!SUPP_INFO}
plot_all_model_fits('NEWS')
```

```{r alternative_models_T, fig.width=5, fig.height=11, echo=!SUPP_INFO}
plot_all_model_fits('TWITTER')
```

`r figcap(TRUE)`
Comparison of six models $S(t)$ for fitting empirical mention frequencies.
_Top (yellow):_ news.
_Bottom (blue):_ Twitter.
...
Fits are nonlinear least-squares fits obtained in log space, i.e., logarithms were taken of both the data and the model $S(t)$ before performing the least-squares optimization (see equation (5) in the paper for the case of the biexponential model).
The biexponential function is defined in equation (4) of the paper.
The other functions are defined as follows:
*Log-normal-inspired:* $S(t) = \exp(a' - b \log t + c (\log t)^2) = a \, t^{-b + c\log t}$. (We call this function "log-normal-inspired" because it takes the functional form of the log-normal distribution, but is not a probability distribution.)
*Power:* $S(t) = a \, t^{-b}$.
*Logarithmic:* $S(t) = a - b \log t$.
*Hyperbolic:* $S(t) = (a + bt)^{-1}$.
*Exponential:* $S(t) = a \, e^{-bt}$.
The Akaike information criterion (AIC), shown in the right plots, captures goodness of fit while accounting for model complexity (smaller AIC values correspond to better fits).
The biexponential model fits the data best in the case of the news (AIC difference = 140), whereas the log-normal-inspired model fits the data best in the case of Twitter (AIC difference = 101).
Note, however, that the log-normal-inspired model is the only one to have the undesirable property of being non-monotonic, unlike the empirical data and unlike what one would require from a theoretical model.
When addressing this issue by constraining the log-normal-inspired model to be monotonically decreasing over the support range of the data (see plot labeled "Log-normal-inspired (constrained)"), the AIC is considerably larger than the AIC of the biexpoential model both in the news and on Twitter.

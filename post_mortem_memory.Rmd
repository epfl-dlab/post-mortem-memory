...
plot_avg_fraction_of_mentioning_docs <- function(medium, model=NULL, model_type=NULL) {
  ...
  if (!is.null(model)) {
    ...
    if (model_type == 'Biexponential') {
      ...
      legend <- c('Biexponential fit S(t) = u(t) + v(t)', 'Communicative memory u(t)', 'Cultural memory v(t)')
    } else if (model_type == 'Bipower') {
      mem_comm <- N * t^-(p+r)
      mem_cult <- N/(p-q+r)*r * (t^-q - t^-(p+r))
      legend <- c('Bipower fit S(t) = u(t) + v(t)', 'Communicative memory u(t)', 'Cultural memory v(t)')
    } else if (model_type == 'Shifted power') {
      mem_comm <- N * t^-p
      mem_cult <- rep(r, length(t))
      legend <- c('Shifted power law S(t) = u(t) + v(t)',
                  expression(paste('Communicative memory u(t) = ', a * t^{-b})),
                  'Cultural memory v(t) = c')
    } else {
...
`r figcap(TRUE)`
Comparison of 10 models $S(t)$ for fitting empirical mention frequencies, for __(a)__ the news and __(b)__ Twitter.
...
By setting $q=0$, $a=\frac{Np}{p+r}$, $b=p+r$, and $c=r$, we obtain the shifted power function as a special case of the bipower function:

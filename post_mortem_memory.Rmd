...
plot_model_fit <- function(medium, name, model, predict, log='') {
  ...
  if (log=='x') text(400, max(y),
                      sprintf('R² = %.3f\nAIC = %.0f', r2, aic),
                      adj=c(1,1), cex=1)
  else text(200, max(y), name, adj=c(0.5,1), cex=1.7)
}
...
run_regression_analysis <- function(medium, ranks) {
  ...
  cat(table, file=sprintf('%s/lm_coef_T_and_N%s.tex', TABLEDIR, suffix), sep="\n")
  return(list(mod_peak=mod_peak, mod_perm=mod_perm))
}

run_regression_analysis_for_death_types <- function(medium, lmdata, ranks) {
  if (medium != 'NEWS' && medium != 'TWITTER') stop('Medium must be \'NEWS\' or \'TWITTER\'')
  lmdata <- if (medium == 'NEWS') lmdata_N else lmdata_T
  ...
  plot_age_coeffs_for_death_types(mod_peak, medium, 'peak_mean_boost', base_age,
                                  if (ranks) c(-0.3,0.6) else c(1.8,4.2))
  plot_age_coeffs_for_death_types(mod_perm, medium, 'perm_boost', base_age,
...
run_regression_analysis_T_vs_N <- function(ranks) {
  ...
  cat(table, file=sprintf('%s/lm_coef_T_vs_N%s.tex', TABLEDIR, suffix), sep="\n")
  return(list(mod_peak=mod_peak, mod_perm=mod_perm))
}
...
__(a) News__
```{r alternative_models_N, fig.width=5, fig.height=10, echo=!SUPP_INFO}
plot_all_model_fits('NEWS')
```

__(b) Twitter__
```{r alternative_models_T, fig.width=5, fig.height=10, echo=!SUPP_INFO, warning=!SUPP_INFO}
plot_all_model_fits('TWITTER')
```

`r figcap(TRUE)`
Comparison of eight models $S(t)$ for fitting empirical mention frequencies, for __(a)__ the news and __(b)__ Twitter.
...
Fits are nonlinear least-squares fits obtained in log space, i.e., logarithms were taken of both the empirical data and the model $S(t)$ before performing the least-squares optimization (see equation (5) in the paper for the case of the biexponential model).
The biexponential function was introduced by Candia et al. ("The universal decay of collective memory and attention." _Nature Human Behaviour_. 2019; 3(1):82-91) and is defined in equation (4) of the paper.
...
The four above functions share the shortcoming of being concave (at best linear, in the case of the power function) when plotted on log-log axes (i.e., $\log S(t)$ is a concave function of $\log t$, cf. right columns), whereas the empirical curves are convex on log-log axes.

Rubin and Wenzel also proposed generalized versions of the hyperbolic and exponential functions, called hyperbolic-power and exponential-power functions, respectively, where $t$ is replaced by the power $t^c$, where $c$ is a third parameter.
...
For $c>0$, the hyperbolic-power and exponential-power functions, too, are concave in log-log space, but when allowing for $b,c < 0$, they can be made convex and are thus better suited for fitting the empirical data (note that, in the following specifications, we maintain $a,b,c > 0$, but replace $b$ by $-b$, and $c$ by $-c$):

* Exponential-power: $S(t) = a \, e^{bt^{-c}}$, i.e., $\log S(t) = \log a + b e^{-c \log t}$.
* Hyperbolic-power: $S(t) = (a - bt^{-c})^{-1}$, i.e., $\log S(t) = -\log (a - be^{-c \log t})$.

Finally, as the seventh function, we consider what Candia et al. refer to as the "log-normal" function, defined as
$S(t) = \exp(\log a - b \log t - c (\log t)^2)$.
To recognize the fact that, although this function takes the functional form of the log-normal distribution, it is not actually used to fit a probability distribution here, we refer to the function as "log-normal-inspired".
The log-normal-inspired function, too, is concave in log-log space, but can be made convex by replacing $c$ by $-c$, i.e.,
$S(t) = \exp(\log a - b \log t + c (\log t)^2)$.
...
* Log-normal-inspired: $S(t) = \exp(\log a + c ((\log t)^2 - 2t_{\max}\log t))$, i.e., $\log S(t) = \log a + c ((\log t)^2 - 2t_{\max}\log t)$.
...
The biexponential model provides the best fit according to both measures ($R^2$ and AIC) and for both media (news and Twitter).
...
mods_N <- run_regression_analysis('NEWS', ranks=FALSE)
mods_T <- run_regression_analysis('TWITTER', ranks=FALSE)
...
par(mfrow=c(2,2))
mods_N <- run_regression_analysis_for_death_types('NEWS', ranks=FALSE)
mods_T <- run_regression_analysis_for_death_types('TWITTER', ranks=FALSE)
```
...
mods <- run_regression_analysis_T_vs_N(ranks=FALSE)
mod_peak <- mods$mod_peak
mod_perm <- mods$mod_perm

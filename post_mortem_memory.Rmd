...
simple_fit <- function(medium) {
  ...
  # Second approximation: q = 0.
  model <- nls(log(y) ~ log( N*t^-(p+s*log(t)) + r ), start=list(p=p0, N=N0, r=0, s=0),
  # model <- nls(log(y) ~ log( N*t^-p + r ), start=list(p=p0, N=N0, r=0),
  # model <- nls(log(y) ~ log( N*exp(-p*t) + r ), start=list(p=p0, N=N0, r=0),
                data=df, control=list(maxiter=1e8), algorithm='port')
  plot(df$t, df$y, log='xy'); lines(df$t, exp(predict(model)), col='green')
  AIC(model); cor(log(df$y), predict(model))^2
  return(model)
}
#################################
